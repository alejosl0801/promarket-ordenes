# MANEJO DE OBJECIONES — PROMARKET ECUADOR

---

## OBJECIONES UNIVERSALES (ambos productos)

### "El precio no me parece real"
> jaja es que el precio es así de bueno! 😄 [argumento específico del producto]
**Regla:** Celebrar la duda, no explicarla. Una explicación larga genera más desconfianza.

### "¿De dónde son? / ¿Son confiables?"
> somos empresa ecuatoriana con bodega en Guayaquil 🇪🇨 trabajamos con Servientrega y el pago es al retirar — no adelanta nada 💛
**Regla:** Esta pregunta es desconfianza disfrazada. Responderla directa y rápido.

### "Lo pienso / Ya le aviso / Ok gracias"
> claro! solo le cuento que el envío está incluido solo por esta temporada 😊 ¿le reservo una unidad mientras decide?
**Regla:** "Ya le aviso" es casi siempre un NO educado. Crear razón para decidir hoy + ofrecer reserva sin compromiso.

### "Es muy caro"
OLLAS: > menos de lo que cuesta un juego de aluminio que se oxida en 6 meses 😊 este es acero inoxidable real con tapas de vidrio incluidas
GUANTE: > una sola sesión de fisioterapia cuesta $30-50 — este guante hace lo mismo en casa todos los días por $59.99 en total 😊

### "¿Puedo ver el producto antes?"
> el pago es al retirar en Servientrega — si al recibir no le convence, no lo retira 😊 cero riesgo

### "¿Puedo revisar antes de pagar?"
> paga justo al momento de retirarlo en Servientrega 😊 — si algo no está bien, no lo retira

### "¿Tienen garantía?"
> 3 meses contra defectos de fábrica ✅ cualquier problema nos escribe y lo resolvemos

### "Mi esposo/esposa decide"
> claro! ¿quiere que le prepare un resumen fácil de mostrarle? 😊

### "No hay Servientrega cerca"
> ¿tiene algún familiar o conocido en una ciudad cercana donde pueda llegar el pedido? 💛

### Pide más descuento del 10%
> la verdad ya le estoy dando el mejor precio que tengo 😊 sin autorización no me da para más 😅

---

## OBJECIONES ESPECÍFICAS — GUANTE

### "¿Funciona de verdad?"
> más de 2.400 pacientes activos en Latinoamérica ya lo usan 😊 y es recomendado por fisioterapeutas y neurólogos

### "¿En qué colores viene?"
> viene en azul con negro, es el color estándar 😊

### "¿Qué incluye exactamente?"
> guante principal + guante espejo + controlador de potencia + pelota de rehabilitación + manual 😊

### "¿Para qué condiciones sirve?"
> para ACV, artritis, parálisis parcial, post-cirugía, rigidez crónica y adultos mayores — cualquier condición que reduzca el movimiento de la mano

### "¿Una unidad o necesito el par?"
> una unidad sirve para la mano que necesita rehabilitación 😊 si quiere trabajar ambas manos le hago precio especial por el par

### "¿Tiene tallas?"
> es talla estándar ajustable a todo tipo de manos 😊

### "¿Tiene izquierdo y derecho?"
> sí, se vende por unidad — ¿cuál necesita? 😊

---

### "No hay respuesta al precio / cliente duda sin decir nada"
Mandar foto real del producto sin texto, solo:
> ¿Ve? 😊
El cliente saca sus propias conclusiones. Una imagen vale más que cualquier explicación.

---

## POR QUÉ NO USAR "75% DE DESCUENTO"

Cuando dices 75% de descuento el cliente piensa:
- "¿Por qué tan barato? ¿Será de mala calidad?"
- "¿Es una estafa?"
- "¿Qué tiene de malo?"

El descuento que debería generar deseo genera desconfianza.

**En cambio decir:**
- "Precio especial de junio: $59.99"
- "Una sesión de fisioterapia cuesta $40 — este guante lo hace todos los días por $59.99 en total"
- "$49.99 precio final — sin letras chiquitas, sin costos ocultos"

**La regla:** No vendas el descuento. Vende el valor. El cliente no quiere sentir que compró algo barato — quiere sentir que hizo una compra inteligente.

---

## PROTOCOLO PARA AUDIOS

Cuando el cliente manda un audio:
1. Escuchar inmediatamente
2. Responder: "escuché! 😊 [respuesta al contenido exacto del audio]"
3. Si no puede escuchar en ese momento: "ya mismo lo escucho, un segundito"
4. NUNCA ignorar un audio — el cliente siente que nadie lo atiende
