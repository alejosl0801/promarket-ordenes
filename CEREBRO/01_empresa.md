# ProMarket Ecuador — PERFIL DE EMPRESA

## Datos básicos
- **Nombre:** ProMarket Ecuador
- **Ubicación:** Loja, Ecuador
- **Bodega/Fulfillment:** Rocket.ec (Guayaquil) — manejo logístico externo
- **Importante:** Al responder "¿de qué parte son?" → decir "somos de Loja 🇪🇨" — NO mencionar Guayaquil como sede propia
- **Canal de venta:** WhatsApp únicamente
- **Plataforma ecommerce:** Rocket.ec
- **Envío:** Servientrega (toda la red nacional)
- **Pago:** Contra entrega — el cliente paga en ventanilla al retirar

## Proceso de envío
1. Cliente confirma pedido y da datos
2. Se genera PDF de orden de retiro (manual)
3. Se empaqueta el producto
4. Servientrega autoriza el envío (siguiente día hábil aprox)
5. Se descarga guía de Servientrega y se envía al cliente
6. Producto llega en 3-4 días hábiles
7. Cliente paga en ventanilla y retira
8. Cliente tiene 5 días para retirar — después se devuelve

## Importante sobre Servientrega
- El cliente paga EN VENTANILLA al momento del retiro
- NO mencionar este proceso — es obvio y genera desconfianza si se explica
- Verificar agencias en servientrega.com.ec o Google
- Si no hay agencia en la ciudad → preguntar si tiene familiar en ciudad cercana

## Descuentos
- Máximo 10%
- NUNCA llamarlo "descuento" — decir "precio especial"
- Par de guantes ya tiene descuento aplicado ($107.98)

## Garantía
- 3 meses contra defectos de fábrica
- Se gestiona por WhatsApp
- Cliente manda foto → se coordina reemplazo

## Reclamos
- Escuchar, dar solución, no ignorar
- Protocolo: "entiendo, déjeme revisarlo ahora mismo"

---

## ESTADO ACTUAL DE CAMPAÑAS
- **Guante:** ACTIVO — presupuesto duplicado (campaña principal)
- **Ollas:** PAUSADO — campañas cerradas. Solo cerrar ventas pendientes en chats existentes, no captar nuevos leads.

## PRECIOS ESPECIALES — PILOTO LIQUIDACIÓN (desde 6 junio 2026)
- **Ollas:** $39.99 — "oferta por liquidación"
- **Guante:** $49.99 — precio especial para leads que no compraron
- Estrategia: escribir a todos los leads anteriores con estos precios para recuperar ventas

---

## PRODUCTOS

### 1. Guante Robot de Rehabilitación (REF: GR-001)
- **Precio:** $59.99 por unidad
- **Par (2 unidades):** $107.98
- **Color:** Azul con negro (color único estándar)
- **Tallas:** S, M, L, XL — solo mencionar si el cliente pregunta
- **Mano:** Izquierda o derecha — se vende por unidad — solo mencionar si el cliente pregunta
- **Tecnología:** Neumática — presión de aire controlada
- **Niveles:** 9 intensidades ajustables
- **Modos:** Automático / Modo Espejo
- **Incluye:** Guante principal + guante espejo + controlador de potencia + pelota de rehabilitación + manual

**Para quién:**
- Derrame cerebral (ACV)
- Parálisis parcial
- Artritis
- Post-cirugía de mano o muñeca
- Rigidez o calambres crónicos
- Adultos mayores

**Argumentos de venta top:**
- Más de 2.400 pacientes activos en Latinoamérica
- Recomendado por fisioterapeutas y neurólogos
- Una sola inversión vs $30-50 por cada sesión de fisioterapia
- A toda hora, sin salir de casa, sin turnos
- 9 niveles — el mismo equipo acompaña todo el proceso
- Un familiar puede asistir desde casa

**Cierre según para quién:**
- Para mamá/abuela → cierre emocional (recuperar movilidad, calidad de vida)
- Para él/ella mismo → cierre funcional (resultados, niveles, progreso)

---

### 2. Juego de Ollas Inoxidable 12 Piezas (REF: OLL-001)
- **Precio:** $49.99
- **Composición:** 6 utensilios (ollas, cacerolas, sartenes) + 6 tapas de vidrio templado
- **Material:** Acero inoxidable de alta calidad
- **Base:** Doble fondo engrosado — calor uniforme
- **Superficie:** Antiadherente
- **Compatibilidad:** Inducción, gas, eléctrica y vitrocerámica
- **Limpieza:** Apto para lavavajillas
- **Kit fijo:** No se pueden sustituir piezas

**Para quién:**
- Cocina nueva desde cero
- Reemplazo de utensilios desgastados
- Quienes buscan calidad profesional accesible
- Cualquier tipo de cocina

**Argumentos de venta top vs competencia:**
- Acero inoxidable real vs aluminio o acero de baja calidad
- Compatible con inducción (la mayoría de sets económicos NO lo son)
- Tapas de vidrio incluidas (otros las venden por separado)
- 12 piezas completas (otros sets son parciales)
- Antiadherente — menos aceite, más fácil de limpiar
- Ahorro de energía con las tapas de vidrio
